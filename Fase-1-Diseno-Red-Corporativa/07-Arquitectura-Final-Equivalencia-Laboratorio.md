...
| VLANs y direccionamiento IP | 12 VLANs, tabla completa (`/24` cada una) | Las **mismas** VLANs, en el mismo `network-inventory.yaml` — no se inventa un esquema aparte para el laboratorio | VLAN tagging real, con 2 hosts físicos en VLANs distintas conectados al mismo switch |
