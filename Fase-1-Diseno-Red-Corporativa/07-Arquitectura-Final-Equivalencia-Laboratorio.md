...
| VLANs y direccionamiento IP | 12 VLANs, tabla completa, máscaras VLSM ajustadas por VLAN (bloque base `172.20.0.0/16`) | Las **mismas** VLANs, en el mismo `network-inventory.yaml` — no se inventa un esquema aparte para el laboratorio | VLAN tagging real, con 2 hosts físicos en VLANs distintas conectados al mismo switch |
